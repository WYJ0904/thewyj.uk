...
class StaticSiteTests(unittest.TestCase):
    ...
    def test_manifest_and_service_worker_shell_are_deployable(self):
        ...
        release_token = "20260926-task24-candidate-r14"
        ...
        self.assertIn('export const APP_VERSION = "2026-09-26-task24-candidate-history-r14"', self.core)
        ...
        self.assertIn('type="module" src="/app.js?v=20260926-task24-candidate-r14"', self.html)
    ...
    def test_task19_design_system_two_contract(self):
        self.assertIn('href="/design-system.css?v=20260926-task24-candidate-r14"', self.html)
        self.assertIn('href="/public-experience.css?v=20260926-task24-candidate-r14"', self.html)
        self.assertIn('href="/workspace-experience.css?v=20260926-task24-candidate-r14"', self.html)
    ...
    def test_payment_ui_uses_protected_blob_qr_and_releases_object_urls(self):
        ...
        self.assertIn("headers: accountSessionHeaders(state.session)", self.app)
    ...
    def test_android_payment_channels_are_accepted_by_the_api(self):
        ...
        rejected with 400 payment_channel_invalid and never reached Finance."""
        model = (ROOT / "functions" / "_lib" / "task21-model.mjs").read_text(encoding="utf-8")
        allowed_match = re.search(r"PAYMENT_CHANNELS = Object\.freeze\(\[(.*?)\]\)", model, re.S)
        self.assertIsNotNone(allowed_match, "PAYMENT_CHANNELS must stay a simple literal list")
        allowed = set(re.findall(r'"([^"]*)"', allowed_match.group(1)))
        parsers = (
            ROOT / "android" / "app" / "src" / "main" / "java" / "uk" / "thewyj" / "app" /
            "task21" / "payment" / "SourceParsers.kt"
        ).read_text(encoding="utf-8")
        channels = set(re.findall(r'override val channel = "([^"]+)"', parsers))
        self.assertGreaterEqual(len(channels), 3, "every parser must declare its channel")
        self.assertTrue(
            channels <= allowed,
            f"Android channels {sorted(channels - allowed)} are not accepted by /api/notification/ingest",
        )
