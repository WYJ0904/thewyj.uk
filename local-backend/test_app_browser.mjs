  ...
  try {
    await check("brief branded startup and unauthenticated navigation", async () => {
      ...
      const pwa = await evaluate(`(async () => {
        ...
        const cachedProductStyles = await caches.match('/product-ui.css?v=20260912-screenshot-r1');
        const cachedDesignStyles = await caches.match('/design-system.css?v=20260912-screenshot-r1');
        const cachedPublicStyles = await caches.match('/public-experience.css?v=20260912-screenshot-r1');
        const cachedWorkspaceStyles = await caches.match('/workspace-experience.css?v=20260912-screenshot-r1');
        const cachedChangelog = await caches.match('/changelog.js?v=20260912-screenshot-r1');
        const cachedLearningSync = await caches.match('/learning-sync.js?v=20260912-screenshot-r1');
        const cachedWorkflows = await caches.match('/workflows.js?v=20260912-screenshot-r1');
      ...
      assert.equal(await evaluate("document.querySelector('#siteVersionLabel').textContent.trim()"), "v2026.09.12.2");
      ...
      assert.equal(await evaluate("localStorage.getItem('wyjChangelogSeenVersion:v1')"), "2026-09-12-task24-1-screenshot-r4");
    ...
    await check("limited anonymous trial stays local and protected routes remain locked", async () => {
      ...
      assert.equal(await evaluate("document.querySelector('#changelogCurrentVersion').textContent.trim()"), "v2026.09.12.1");
      assert.equal(await evaluate("document.querySelector('#versionNotice').classList.contains('hidden')"), true);
