  ...
  try {
    await check("brief branded startup and unauthenticated navigation", async () => {
      ...
      const pwa = await evaluate(`(async () => {
        ...
        const cachedProductStyles = await caches.match('/product-ui.css?v=20260912-task24-3-r1');
        const cachedDesignStyles = await caches.match('/design-system.css?v=20260912-task24-3-r1');
        const cachedPublicStyles = await caches.match('/public-experience.css?v=20260912-task24-3-r1');
        const cachedWorkspaceStyles = await caches.match('/workspace-experience.css?v=20260912-task24-3-r1');
        const cachedChangelog = await caches.match('/changelog.js?v=20260912-task24-3-r1');
        const cachedLearningSync = await caches.match('/learning-sync.js?v=20260912-task24-3-r1');
        const cachedWorkflows = await caches.match('/workflows.js?v=20260912-task24-3-r1');
