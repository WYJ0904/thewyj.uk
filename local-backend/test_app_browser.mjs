  ...
  try {
    await check("brief branded startup and unauthenticated navigation", async () => {
      ...
      const pwa = await evaluate(`(async () => {
        ...
        const cachedProductStyles = await caches.match('/product-ui.css?v=20260926-task24-candidate-r14');
        const cachedDesignStyles = await caches.match('/design-system.css?v=20260926-task24-candidate-r14');
        const cachedPublicStyles = await caches.match('/public-experience.css?v=20260926-task24-candidate-r14');
        const cachedWorkspaceStyles = await caches.match('/workspace-experience.css?v=20260926-task24-candidate-r14');
        const cachedChangelog = await caches.match('/changelog.js?v=20260926-task24-candidate-r14');
        const cachedLearningSync = await caches.match('/learning-sync.js?v=20260926-task24-candidate-r14');
        const cachedWorkflows = await caches.match('/workflows.js?v=20260926-task24-candidate-r14');
