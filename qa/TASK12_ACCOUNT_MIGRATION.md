...
- Cloud target: D1 tables created by `cloudflare/migrations/0003_accounts_sessions.sql`.
...
Password change, admin reset, ban/unban, forced logout and deletion increment `session_version` and revoke all D1 sessions. Session resolution checks digest, expiry, revocation, account state and version on every request. A user has at most 12 active sessions.
